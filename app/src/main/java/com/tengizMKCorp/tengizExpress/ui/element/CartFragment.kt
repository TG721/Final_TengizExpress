    ...
    private fun handleSwipe() {
        ...
        val swipeGesture = object : SwipeGesture(requireContext()){
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                productName = ((viewHolder.itemView as androidx.constraintlayout.widget.ConstraintLayout).getChildAt(1) as TextView).text.toString()
                discountedPrice = ((viewHolder.itemView as androidx.constraintlayout.widget.ConstraintLayout).getChildAt(2) as TextView).text.toString()
                productID = ((viewHolder.itemView as androidx.constraintlayout.widget.ConstraintLayout).getChildAt(4) as TextView).text.toString()
                productPicture = ((viewHolder.itemView as androidx.constraintlayout.widget.ConstraintLayout).getChildAt(5) as TextView).text.toString()
                currentCartProduct = CartModel(productID, discountedPrice.toDouble(), productName,productPicture)
